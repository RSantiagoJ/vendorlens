...
- [ ] What each AI agent actually does (extract → risk → score → memo → negotiation)
- [ ] Why five agents instead of one big prompt (per-stage specialization, better structured output)
- [ ] The negotiation playbook — what's in a brief (tactics, red lines, BATNA), where the data comes from (risk flags + scores)
